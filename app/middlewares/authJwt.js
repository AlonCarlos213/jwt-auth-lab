...
export const isAdmin = async (req, res, next) => {
  try {
    ...
    res.status(403).json({ message: "⛔️ ¡Se requiere el rol de administrador!" });
  } catch (error) {
...
export const isModerator = async (req, res, next) => {
  try {
    ...
    res.status(403).json({ message: "⛔️ ¡Se requiere el rol de moderador!" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
export const isModeratorOrAdmin = async (req, res, next) => {
  try {
    ...
    res.status(403).json({ message: "⛔️ ¡Se requiere el rol de moderador o administrador!" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
